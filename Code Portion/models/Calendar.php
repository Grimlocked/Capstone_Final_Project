<?php
	...
	class Calendar {
		...
		private $timeLabels = array("07:00 AM", "08:00 AM", "09:00 AM", "10:00 AM", "11:00 AM", "12:00 PM", "01:00 PM", "02:00 PM", "03:00 PM", "04:00 PM", "05:00 PM", "06:00 PM", "07:00 PM", "08:00 PM", "09:00 PM", "10:00 PM", "11:00 PM", "12:00 PM");
				...
				for($d = 0; $d < count($this->datesOfTheWeek); $d++){
					...
					if($event['reservedDate'] == date('Y-m-d', strtotime($this->datesOfTheWeek[$d]))){	
						...
						if(date('G', strtotime($event['endtime'])) > 7){
							...
							if(date('i', strtotime($event['endtime'])) > 0){
	...
	$calendarContent .= '
	    ...
	    private function createWeekNavigation($weeksInMonth){
	        ...
	        $headerContent .= '<div class="calendarNavBox">';
		        $headerContent .= '<a href="./?action=weekView&Year=' . date('Y', strtotime($previousDate)) . '&month=' . date('n', strtotime($previousDate)) . '&date=' . date('j', strtotime($previousDate)) . '" class="waves-effect waves-light btn">Previous</a>';
		        $headerContent .= "<div class='centerAlign' >" . date('M Y',strtotime($this->currentYear.'-'.$this->currentMonth.'-1'));
		        $headerContent .= ' | <a  href="./?action=weekView" class="waves-effect waves-light btn center">Today</a></div>';
		       	$headerContent .= '<a href="./?action=weekView&Year=' . date('Y', strtotime($nextDate)) . '&month=' . date('n', strtotime($nextDate)) . '&date=' . date('j', strtotime($nextDate)) . '" class="waves-effect waves-light btn rightAlign">Next</a>';
		    $headerContent .= '</div>';
	    ...
	    private function createWeekDayBar(){
	    	...
	    	for($u = 0; $u < count($this->datesOfTheWeek); $u++){
	    		...
	    		if(($u == $this->GetDayOfWeek(date('j', time()), date('n', time()), date('Y', time()))) && $this->date == date('j', time())){
					$dayBar .= '<th class="today center">';
	    		}
	    		else{
	    			$dayBar .= '<th class="center">';
	    		}
